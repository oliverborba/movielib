...
class MovieViewController: UIViewController {
    ...
    func loadTrailer(with title: String){
        ...
        guard let encodedTitle = title.addingPercentEncoding(withAllowedCharacters: .urlHostAllowed), let url = URL(string: "\(itunesPath)\(encodedTitle)") else {return}
        URLSession.shared.dataTask(with: url){(data, _, _) in
            let apiResult = try! JSONDecoder().decode(ItunesResult.self, from: data!)
            self.trailer = apiResult.results.first?.previewUrl ?? ""
            self.prepareVideo()}.resume()
                }
    
    func prepareVideo(){
        guard let url = URL(string: trailer)
        else{return}
